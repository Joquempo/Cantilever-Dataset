...
fig_top_opt     = True  # optimized topology and discretized optimized topology
fig_top_sen     = True  # topology vectors and sensitivity vectors
fig_dis         = True  # displacements vectors
fig_obj_vol_gra = True  # objective function, volume and gray level
...
if not os.path.exists('./dis'):
    os.mkdir('./dis')
if not os.path.exists('./obj_vol_gra'):
    os.mkdir('./obj_vol_gra')
...
while (file < file_lim) and (os.path.exists(rpath + 'f{:04d}'.format(file))):
    ...
    # optimized topology and discretized optimized topology
    if fig_top_opt:
        if not os.path.exists(rpath + 'f{:04d}/top_p1_opt.npy'.format(file)):
            print('missing : f{:04d}/top_p1_opt.npy'.format(file))
            sys.exit()
        list_top_p1_opt = np.load(rpath + 'f{:04d}/top_p1_opt.npy'.format(file))
        if not os.path.exists(rpath + 'f{:04d}/dtop_opt.npy'.format(file)):
            print('missing : f{:04d}/dtop_opt.npy'.format(file))
            sys.exit()
        list_dtop_opt = np.load(rpath + 'f{:04d}/dtop_opt.npy'.format(file))
    
    # topology vectors
    if fig_top_sen or fig_dis:
        if not os.path.exists(rpath + 'f{:04d}/top_p1.npy'.format(file)):
            print('missing : f{:04d}/top_p1.npy'.format(file))
            sys.exit()
        list_top_p1 = np.load(rpath + 'f{:04d}/top_p1.npy'.format(file))
        
    # sensitivity vectors
    if fig_top_sen:
        if not os.path.exists(rpath + 'f{:04d}/sen_p1.npy'.format(file)):
            print('missing : f{:04d}/sen_p1.npy'.format(file))
            sys.exit()
        list_sen_p1 = np.load(rpath + 'f{:04d}/sen_p1.npy'.format(file))
    ...
    # objective function, volume and gray level
    if fig_obj_vol_gra:
        ...
        if not os.path.exists(rpath + 'f{:04d}/vol_p1.npy'.format(file)):
            print('missing : f{:04d}/vol_p1.npy'.format(file))
            sys.exit()
        list_vol_p1 = np.load(rpath + 'f{:04d}/vol_p1.npy'.format(file))
        if not os.path.exists(rpath + 'f{:04d}/gra_p1.npy'.format(file)):
            print('missing : f{:04d}/gra_p1.npy'.format(file))
            sys.exit()
        list_gra_p1 = np.load(rpath + 'f{:04d}/gra_p1.npy'.format(file))
    ...
    # optimized topology and discretized optimized topology
    if fig_top_opt:
        print(': : optimized topology and discretized optimized topology...')
        for k in range(len(list_fid)):
            ...
            ax[0].imshow(xmat,cmap='gray_r',vmin=0,vmax=1.0,origin='lower')
            ax[0].axis('off')
            x = list_dtop_opt[k]
            x = np.unpackbits(x,axis=None).astype(float)
            xmat = np.reshape(x,(Ny,Nx),order='F')
            xmat = np.concatenate((c0,xmat,c1),axis=1)
            ax[1].imshow(xmat,cmap='gray_r',vmin=0,vmax=1.0,origin='lower')
            ax[1].axis('off')
    ...
    if fig_dis:
        ...
        for k in range(len(list_fid)):
            ...
            for kk in range(list_ptr2opt[k],list_ptr2opt[k+1]):
                ...
                ax.set_ylim([ymin-0.01*Dy,ymax+0.01*Dy])
                ax.axis('off')
    ...
    # objective function, volume and gray level
    if fig_obj_vol_gra:
        print(': : objective function, volume and gray level...')
        for k in range(len(list_fid)):
            fid = list_fid[k]
            plt.figure(num=0).clear()
            ...
            ax[0,0].plot(obj,'ok-',linewidth=2)
            ax[0,0].axis([-0.75, len(obj)-0.25, miny, maxy])
            ax[0,0].set_ylabel('compliance [J]',fontsize=18)
            ax[0,0].grid()
            delta = max(obj[16:]) - min(obj)
            miny = min(obj)-0.02*delta
            maxy = max(obj[16:])+0.02*delta
            ax[1,0].plot(obj,'ok-',linewidth=2)
            ax[1,0].axis([-0.75, len(obj)-0.25, miny, maxy])
            ax[1,0].set_ylabel('compliance [J]',fontsize=18)
            ax[1,0].set_xlabel('iteration',fontsize=18)
            ax[1,0].grid()
            vol = list_vol_p1[list_ptr2opt[k]:list_ptr2opt[k+1]]
            ax[0,1].plot(vol,'ok-',linewidth=2)
            ax[0,1].axis([-0.75, len(obj)-0.25, -0.05, 1.05])
            ax[0,1].set_ylabel('volume fraction',fontsize=18)
            ax[0,1].grid()
            gra = list_gra_p1[list_ptr2opt[k]:list_ptr2opt[k+1]]
            ax[1,1].plot(gra,'ok-',linewidth=2)
            ax[1,1].axis([-0.75, len(obj)-0.25, -0.05, 1.05])
            ax[1,1].set_ylabel('gray level',fontsize=18)
            ax[1,1].set_xlabel('iteration',fontsize=18)
            ax[1,1].grid()
            fig.set_size_inches(16, 9)
            fig.savefig('./obj_vol_gra/f{:06d}.png'.format(fid),bbox_inches='tight',pad_inches=0.05,dpi=100)
