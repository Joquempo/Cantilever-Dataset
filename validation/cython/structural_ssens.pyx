...
cdef void sens(double [:] alpha_r, double [:] dens, double [:,::1] dKe, double [:] ug, double p, long long Nx, long long Ny):
    cdef long long N
    cdef long long e
    cdef long long k1
    cdef long long k2
...
def str_ssens(alpha_r, dens, dKe, ug, p, Nx, Ny, serial=False):
    if serial:
        sens_serial(alpha_r, dens, dKe, ug, p, Nx, Ny)
    else:    
        sens(alpha_r, dens, dKe, ug, p, Nx, Ny)
    return