...
cdef void sfilter(double [:] alpha_r, double [:] alpha_f, double rm, long long Nx, long long Ny):
    cdef long long N
    cdef long long e
    N = Nx*Ny
    if rm <= 1.0:
    ...
    for e in prange(N,nogil=True,schedule='static'):
...
cdef void sfilter_noload(double [:] alpha_r, double [:] alpha_f, double rm, long long Nx, long long Ny, long long [:] ld_lim):
    cdef long long N
    cdef long long e
    cdef long long v0
    cdef long long v1
...
def str_filter(alpha_r, alpha_f, rmax, esize, Nx, Ny, load_lim=None, serial=False):
    rm = rmax/esize
    if serial:
        if load_lim is None:
        ...
        if load_lim is None:
            sfilter(alpha_r, alpha_f, rm, Nx, Ny)
        else:    
            sfilter_noload(alpha_r, alpha_f, rm, Nx, Ny, load_lim)
    return